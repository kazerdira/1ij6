...
class InputValidator:
    ...
    async def validate_audio_file(file: UploadFile) -> bytes:
        ...
        # 5. Validate MIME type using magic bytes (optional - requires python-magic)
        try:
            import magic
            ...
            # Magic detection failed - continue with extension validation only
            pass
...
class TranslationRequest(BaseModel):
    ...
    source_language: str = Field(..., pattern=r'^[a-z]{2,3}(_[A-Za-z]{4})?$')
    target_language: str = Field(..., pattern=r'^[a-z]{2,3}(_[A-Za-z]{4})?$')
    ...
    def validate_language(cls, v, info):
        return InputValidator.validate_language_code(v, info.field_name)


class TranscriptionConfig(BaseModel):
    """Validated transcription configuration"""
    source_language: str = Field("ko", pattern=r'^[a-z]{2,3}(_[A-Za-z]{4})?$')
    target_language: str = Field("eng_Latn", pattern=r'^[a-z]{2,3}(_[A-Za-z]{4})?$')
